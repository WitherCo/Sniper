...
export default {
  ...
  async execute(interaction: CommandInteraction | Message) {
    if (interaction instanceof CommandInteraction) {
      ...
      return await interaction.reply({ content: `✅ Moderation action completed: View a user's moderation history\nTarget: ${target}\nReason: ${reason}`, ephemeral: true });
    } else {
      const args = interaction.content.split(" ").slice(1);
      const target = args[0] || "No user specified";
      const reason = args.slice(1).join(" ") || "No reason specified";
      return await interaction.reply(`✅ Moderation action completed: View a user's moderation history\nTarget: ${target}\nReason: ${reason}`);
    }
  }
} as DiscordCommand;
